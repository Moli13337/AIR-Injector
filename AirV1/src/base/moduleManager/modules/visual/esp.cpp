...
namespace
{
	void DrawOutlinedText(ImDrawList* drawList, ImFont* font, float size, const ImVec2& pos, ImU32 color, const char* text)
	{
		const ImU32 outline = ImColor(0, 0, 0, 220);
		drawList->AddText(font, size, ImVec2(pos.x - 1.0f, pos.y), outline, text);
		drawList->AddText(font, size, ImVec2(pos.x + 1.0f, pos.y), outline, text);
		drawList->AddText(font, size, ImVec2(pos.x, pos.y - 1.0f), outline, text);
		drawList->AddText(font, size, ImVec2(pos.x, pos.y + 1.0f), outline, text);
...
void Esp::RenderOverlay()
{
	...
	for (Data data : m_renderData)
	{
		...
		if (settings::ESP_BoxType == 0)
		{
			...
			{
				ImColor bottomColor = ImColor(0.0f, 0.0f, 0.0f, 0.10f * data.opacityFadeFactor);
				ImColor bottomColorFriend = ImColor(0.0f, 0.20f, 0.08f, 0.12f * data.opacityFadeFactor);
				ImColor topColor = ImColor(0.0f, 0.0f, 0.0f, 0.03f * data.opacityFadeFactor);
				ImColor topColorFriend = ImColor(0.0f, 0.35f, 0.16f, 0.05f * data.opacityFadeFactor);
			...
			if (settings::ESP_Box)
			{
				ImColor color = ImColor(185, 214, 190, static_cast<int>(170 * data.opacityFadeFactor));
				ImColor colorFriend = ImColor(70, 255, 130, static_cast<int>(200 * data.opacityFadeFactor));
				ImGui::GetWindowDrawList()->AddRect(ImVec2(left, top), ImVec2(right, bottom), IsFriend ? colorFriend : color, 0.0f, 0, 1.0f);
			}

			if (settings::ESP_Outline)
			{
				ImColor color = ImColor(0, 0, 0, static_cast<int>(115 * data.opacityFadeFactor));
				ImGui::GetWindowDrawList()->AddRect(ImVec2(left - 1, top - 1), ImVec2(right + 1, bottom + 1), color, 0.0f, 0, 1.0f);
				ImGui::GetWindowDrawList()->AddRect(ImVec2(left + 1, top + 1), ImVec2(right - 1, bottom - 1), color, 0.0f, 0, 1.0f);
			}

			ImFont* nameFont = Menu::boldFont18 ? Menu::boldFont18 : ImGui::GetFont();
			const float nameSize = 18.0f;
			const ImVec2 nameTextSize = nameFont->CalcTextSizeA(nameSize, FLT_MAX, 0.0f, data.name.c_str());
			const ImVec2 namePos((left + right - nameTextSize.x) * 0.5f, top - nameTextSize.y - 3.0f);
			DrawOutlinedText(ImGui::GetWindowDrawList(), nameFont, nameSize, namePos, ImColor(92, 255, 255, static_cast<int>(255 * data.opacityFadeFactor)), data.name.c_str());
		}
		...
		if (settings::ESP_HealthBar)
		{
			...
			float healthBarGap = 5.0f;
			float healthBarLeft = left - healthBarWidth - healthBarGap;
			
			ImGui::GetWindowDrawList()->AddRect(
				ImVec2(healthBarLeft - 1.0f, top - 1.0f),
				ImVec2(healthBarLeft + healthBarWidth + 1.0f, bottom + 1.0f),
				ImColor(0, 0, 0, static_cast<int>(210 * data.opacityFadeFactor)),
				0.0f,
				0,
				1.0f
			);
			...
			ImGui::GetWindowDrawList()->AddRectFilled(
				ImVec2(healthBarLeft, top), 
				ImVec2(healthBarLeft + healthBarWidth, bottom), 
				ImColor(12, 12, 12, static_cast<int>(150 * data.opacityFadeFactor))
			);
			...
			float healthHeight = diff * scaleFactor;
			float barTop = bottom - healthHeight;
			
			for (float y = barTop; y < bottom; y += 1.0f)
			{
				float healthRatio = (y - top) / diff;
				float healthValue = (1.0f - healthRatio) * 20.0f;
				float r = (20.0f - healthValue) / 20.0f * 255.0f;
				float g = healthValue / 20.0f * 255.0f;
				float b = 0.0f;
				
				ImGui::GetWindowDrawList()->AddLine(
					ImVec2(healthBarLeft, y),
					ImVec2(healthBarLeft + healthBarWidth, y),
					ImColor((int)r, (int)g, (int)b, (int)(255 * data.opacityFadeFactor)),
					1.0f
				);
			}
		}
