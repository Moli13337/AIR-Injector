...
void Radar::RenderHud()
{
	...
	// Set window background and rounding - use theme color
	ImVec4 themeColor;
	ImGui::ColorConvertHSVtoRGB(settings::Menu_ThemeHue, 0.75f, 0.15f, themeColor.x, themeColor.y, themeColor.z);
	themeColor.w = 0.85f;
	ImGui::PushStyleColor(ImGuiCol_WindowBg, themeColor);
	ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, settings::Radar_Size / 2); // Circular margin
	...
	ImGui::Begin("RaderHudWindow", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | windowFlags);
	{
		...
		settings::Radar_Position[0] = windowPos.x;
		settings::Radar_Position[1] = windowPos.y;
		settings::Radar_Size = windowSize.x;

		ImVec2 zerozero = ImVec2(windowSize.x / 2, windowSize.y / 2);

		// Draw cross in center
		ImVec4 themeColor;
		ImGui::ColorConvertHSVtoRGB(settings::Menu_ThemeHue, 0.75f, 0.6f, themeColor.x, themeColor.y, themeColor.z);
		ImU32 crossColor = ImGui::ColorConvertFloat4ToU32(themeColor);
		drawList->AddLine(ImVec2(zerozero.x - 5, zerozero.y), ImVec2(zerozero.x + 5, zerozero.y), crossColor, 1.0f);
		drawList->AddLine(ImVec2(zerozero.x, zerozero.y - 5), ImVec2(zerozero.x, zerozero.y + 5), crossColor, 1.0f);

		// Draw direction labels (N, S, E, W)
		const char* directions[] = { "N", "S", "E", "W" };
		ImVec2 dirPositions[] = {
			ImVec2(zerozero.x, zerozero.y - windowSize.y / 2 + 10), // N
			ImVec2(zerozero.x, zerozero.y + windowSize.y / 2 - 20), // S
			ImVec2(zerozero.x + windowSize.x / 2 - 20, zerozero.y), // E
			ImVec2(zerozero.x - windowSize.x / 2 + 10, zerozero.y)  // W
		};

		for (int i = 0; i < 4; i++)
		{
			ImVec2 textSize = ImGui::CalcTextSize(directions[i]);
			drawList->AddText(ImVec2(dirPositions[i].x - textSize.x / 2, dirPositions[i].y - textSize.y / 2), ImColor(255, 255, 255, 200), directions[i]);
		}
		...
		for (Data data : m_renderData)
		{
			...
			else
			{
				...
				// Draw other players as arrows
				ImVec2 p1 = ImVec2(0, -4);
				ImVec2 p2 = ImVec2(-3, 3);
				ImVec2 p3 = ImVec2(3, 3);

				// Rotate arrow points based on player yaw
				if (!settings::Radar_RotateWithPlayer)
				{
					p1 = rotatePoint(p1.x, p1.y, m_playerYaw);
					p2 = rotatePoint(p2.x, p2.y, m_playerYaw);
				...
				// Translate arrow to radar position
				p1.x += radarPos.x; p1.y += radarPos.y;
				p2.x += radarPos.x; p2.y += radarPos.y;
				p3.x += radarPos.x; p3.y += radarPos.y;

				drawList->AddTriangleFilled(p1, p2, p3, configmanager::IsFriend(data.name) ? friendColor : playerColor);
