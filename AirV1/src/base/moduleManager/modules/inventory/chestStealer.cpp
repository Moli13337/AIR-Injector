...
void ChestStealer::RenderItems()
{
	...
	if (ImGui::BeginPopup("Add Item Popup"))
	{
		...
		// Filtered block list
		if (ImGui::BeginListBox("##blockList", ImVec2(ImGui::GetContentRegionAvail().x, 200)))
		{
	...
	// Render items inline in the main panel
	ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10.f);
	Menu::HorizontalSeparator("CS_Sep");
	ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10.f);

	Menu::BoldText("Items", FontSize::SIZE_18);
	ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10.f);

	// Items list
	if (ImGui::BeginChild("##ItemsList", ImVec2(ImGui::GetContentRegionAvail().x, 150), true))
	{
		...
		for (int i = 0; i < settings::CS_Items.size(); ++i)
		{
			...
			Menu::Button(itemName.c_str(), ImVec2(ImGui::GetContentRegionAvail().x - deleteBtnSize.x - (scrollbar ? ImGui::GetStyle().ScrollbarSize : 0.f) - 10.f, 0.f));
	...
	// Add item button
	if (Menu::MenuButton("Add Item", ImVec2(ImGui::GetContentRegionAvail().x, 25.f), FontSize::SIZE_16))
	{
		showAddPopup = true;
	}
}
