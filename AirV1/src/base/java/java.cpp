...
static bool checkSilent189()
{
    jclass minecraftClass;
    Java::AssignClass("net.minecraft.client.Minecraft", minecraftClass);
    if (!minecraftClass) return false;

    // Silent Client uses standard obfuscation but has specific markers
    jmethodID getMinecraftMethod = Java::env->GetStaticMethodID(minecraftClass, "getMinecraft", "()Lnet/minecraft/client/Minecraft;");
    if (!getMinecraftMethod) return false;
    ...
    // Check for Silent Client specific fields or methods
    jfieldID launchedVersionField = Java::env->GetFieldID(minecraftClass, "launchedVersion", "Ljava/lang/String;");
    if (!launchedVersionField) return false;
    ...
    // Silent Client typically doesn't modify the version string but has other markers
    // Check for Silent Client brand
    jclass clientBrandRetrieverClass = Java::env->FindClass("net/minecraft/client/ClientBrandRetriever");
    if (clientBrandRetrieverClass) {
        jmethodID getClientModNameMethod = Java::env->GetStaticMethodID(clientBrandRetrieverClass, "getClientModName", "()Ljava/lang/String;");
        if (getClientModNameMethod) {
            jobject brand = Java::env->CallStaticObjectMethod(clientBrandRetrieverClass, getClientModNameMethod);
            if (brand) {
                std::string brandStr = String(brand).ToString();
                if (brandStr.find("Silent") != std::string::npos || brandStr.find("silent") != std::string::npos) {
                    if (version.find("1.8.9") != std::string::npos) { Java::version = MinecraftVersion::SILENT_1_8_9; return true; }
                }
            }
        }
    }

    // Alternative detection: check for Silent-specific classes
    jclass silentClass = Java::env->FindClass("net/silentclient/...");
    if (silentClass) {
        if (version.find("1.8.9") != std::string::npos) { Java::version = MinecraftVersion::SILENT_1_8_9; return true; }
    }
...
static bool checkBadlion189()
{
    ...
    // Check for Badlion Client brand
    jclass clientBrandRetrieverClass = Java::env->FindClass("net/minecraft/client/ClientBrandRetriever");
    if (clientBrandRetrieverClass) {
        jmethodID getClientModNameMethod = Java::env->GetStaticMethodID(clientBrandRetrieverClass, "getClientModName", "()Ljava/lang/String;");
        if (getClientModNameMethod) {
            ...
            if (brand) {
                std::string brandStr = String(brand).ToString();
                if (brandStr.find("Badlion") != std::string::npos || brandStr.find("badlion") != std::string::npos) {
                    if (version.find("1.8.9") != std::string::npos) { Java::version = MinecraftVersion::BADLION_1_8_9; return true; }
                }
            }
        }
    }

    // Alternative detection: check for Badlion-specific classes
    jclass badlionClass = Java::env->FindClass("net/badlion/...");
    if (badlionClass) {
        if (version.find("1.8.9") != std::string::npos) { Java::version = MinecraftVersion::BADLION_1_8_9; return true; }
    }

    // Check for mod classes that Badlion uses
    jclass modClass = Java::env->FindClass("cc/badlion/mod/...");
    if (modClass) {
        if (version.find("1.8.9") != std::string::npos) { Java::version = MinecraftVersion::BADLION_1_8_9; return true; }
    }
...
void Java::GetMinecraftVersion()
{
	...
	if (checkSilent189()) return;
	if (checkBadlion189()) return;
