...
void Base::Init()
{
	...
	case MinecraftVersion::FORGE_1_7_10:
		...
		break;
	case MinecraftVersion::SILENT_1_8_9:
		LOG_INFO("Silent Client 1.8.9 detected");
		break;
	case MinecraftVersion::BADLION_1_8_9:
		LOG_INFO("Badlion Client 1.8.9 detected");
		break;
	case MinecraftVersion::UNKNOWN:
		LOG_INFO("Unknown Minecraft version detected");
		MessageBoxA(nullptr, "Unknown Minecraft version detected", "AirV1", MB_OK | MB_ICONERROR);
		Logger::Shutdown();
		MH_Uninitialize();
		Java::Shutdown();
	...
	if (!HotSpot::Init())
	{
		...
		MessageBoxA(nullptr, "Failed to initialize HotSpot", "AirV1", MB_OK | MB_ICONERROR);
		Logger::Shutdown();
		MH_Uninitialize();
		Java::Shutdown();
		FreeLibraryAndExitThread(Main::hModule, 0);
