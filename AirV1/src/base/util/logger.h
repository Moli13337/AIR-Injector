...
struct Logger
{
	...
	static void Init()
	{
		// Don't create console window to avoid separate window
		// AllocConsole();
		...
		std::string logFilePath = configmanager::GetAirV1Path() + "\\log.txt";
	...
	static void Log(LogLevel level, const char* format, ...)
	{
		...
		// Don't output to console to avoid separate window
